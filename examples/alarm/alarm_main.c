...
// //   fd = open(CONFIG_EXAMPLES_ALARM_DEVPATH, O_RDWR);
// //   if (fd < 0)
// //     {
// //       int errcode = errno;
...
//     // Set up the alarm time (for example, 5 seconds from now)
...
//     alarm_time.tm_mon = 0;
//     alarm_time.tm_year = 2025;

//     // Set up the alarm structure
//     alarm.id = 0;  // First alarm (you can set multiple alarms)
//     alarm.pid = 0;  // Notify the caller task (or set a different PID)
//     alarm.event.sigev_notify = SIGEV_SIGNAL;  // Notify with a signal
//     alarm.event.sigev_signo = SIGALRM;        // Signal type
//     alarm.event.sigev_value.sival_ptr = NULL; // Optional: pass data
//     alarm.time = alarm_time;

//     // Set the alarm
//     ret = ioctl(fd, RTC_SET_ALARM, (unsigned long)&alarm);
//     if (ret < 0)
//     {
...
//     // Wait for the alarm to trigger (or other tasks to execute)
//     while (1)
//     {
//         sleep(1);
//     }
...
#define TIMER_DEVICE "/dev/timer6" // Timer device path

// Signal handler when timer expires
...
{
    int timer_fd;
    uint32_t timeout = 1000000; // Timeout in microseconds (1 second)
    ...
    ret = ioctl(timer_fd, TCIOC_SETTIMEOUT, timeout);
    if (ret < 0)
    {
    ...
    // Configure notification on expiration (signal)
    memset(&notify, 0, sizeof(notify));
    notify.pid = getpid(); // Current process
    notify.event.sigev_notify = SIGEV_SIGNAL;
    notify.event.sigev_signo = SIGUSR1; // Signal to send
    ret = ioctl(timer_fd, TCIOC_NOTIFICATION, (unsigned long)&notify);
    if (ret < 0)
    {
        perror("Failed to configure notification");
        close(timer_fd);
        return 1;
    }
    ...
    // Wait for the timer to expire (handled by signal)
    while (1)
    {
        pause(); // Wait for signal (timer expiration)
    }

    // Clean up (this part will never be reached in this case)
    close(timer_fd);
    return 0;
}
